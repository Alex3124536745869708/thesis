...
def calculate_mean_std_min_max():
    ...
    for agent in agent_list:
        ...
        LCTest = LearningCurvePlot(title=f"Both environments {agent} ε-greedy learning curve") # Initialize plot and give title
        LCTest.ax.set_xlabel("Timesteps") # Set x-axis label
        LCTest.ax.set_ylabel("Average evaluation reward") # Set y-axis label

        LCtime = LearningCurvePlot(title=f"Both environments {agent} ε-greedy time") # Initialize plot and give title
        LCtime.ax.set_xlabel("Timesteps") # Set x-axis label
        LCtime.ax.set_ylabel("Average time duration") # Set y-axis label
        ...
        for env in env_list:
            for epsilon in epsilon_list:
                for alpha in alpha_list:
                    ...
                    # min and max are not used for the graphs
                    ...
                    LCTest.add_curve(y=smooth(mean,window=smoothing_window),std=std,x=df_r["indx"],\
                                        label=f"env = {env}, α = {alpha}, ε= {epsilon}",annotation=False) # add reward curve to plot
                    ...
                    if agent == "Dyna_LLM":
                        ...
                        col_length = n_repetitions # amount of columns of one experiment of one alpha, epsilon and environment
                    start_col_idx = (col_length*counter)
                    end_col_idx = start_col_idx+col_length
                    ...
                    LCtime.add_curve(y=smooth(mean,window=smoothing_window),std=std,\
                                        label=f"env = {env}, α = {alpha}, ε= {epsilon}",annotation=False) # add time curve to plot
                    #update counter
                    counter+=1
        ...
        if agent == "Q-learning":
            df_r.to_csv("new_q_r.csv", header=True, index=False) # reward
            df_time.to_csv("new_q_time.csv", header=True, index=False) # time
        elif agent == "Dyna_Q":
            df_r.to_csv("new_dyna_r.csv", header=True, index=False) # reward
            df_time.to_csv("new_dyna_time.csv", header=True, index=False) # time
        else: #Dyna_LLM
            df_r.to_csv("new_llm_r.csv", header=True, index=False) # reward
            df_time.to_csv("new_llm_time.csv", header=True, index=False) # time
...
def individual_agent_env_graphs():
    for agent in agent_list:
        df_r, df_time = select_result_data(agent)
        for env in env_list:
            LCTest = LearningCurvePlot(title=f"{agent} ε-greedy learning curve") # Initialize plot and give title
            LCTest.ax.set_xlabel("Timesteps") # Set x-axis label
            LCTest.ax.set_ylabel("Average evaluation reward") # Set y-axis label

            LCtime = LearningCurvePlot(title=f"{agent} ε-greedy time") # Initialize plot and give title
            LCtime.ax.set_xlabel("Timesteps") # Set x-axis label
            LCtime.ax.set_ylabel("Average time duration") # Set y-axis label
            ...
            for epsilon in epsilon_list:
                for alpha in alpha_list:
                    ...
                    LCTest.add_curve(y=smooth(mean,window=smoothing_window),std=std,x=df_r["indx"],\
                                        label=f"α = {alpha}, ε= {epsilon},",annotation=False) # add reward curve to plot
...
def make_comparison_per_env_graphs():
    ...
    for env in env_list:
        for combination in agent_combinations:
            ...
            LCTest.ax.set_xlabel("Timesteps") # Set x-axis label
            LCTest.ax.set_ylabel("Average evaluation reward") # Set y-axis label
            LCtime = LearningCurvePlot(title=f"{title} ε-greedy time") # Initialize plot and give title
            LCtime.ax.set_xlabel("Timesteps") # Set x-axis label
            LCtime.ax.set_ylabel("Average time duration") # Set y-axis label
            ...
            for agent in combination:
                ...
                for epsilon in epsilon_list:
                    for alpha in alpha_list:
                        ...
                        std = df_r[f"{env},{epsilon},{alpha},std"]
                        LCTest.add_curve(y=smooth(mean,window=smoothing_window),std=std,x=df_r["indx"],\
                                            label=f"agent = {agent}, α = {alpha}, ε= {epsilon},",annotation=False) # add reward curve to plot
                        
                        # -----------for the timeduration dataframes-----------
                        mean= df_time[f"{env},{epsilon},{alpha},mean"]
                        std = df_time[f"{env},{epsilon},{alpha},std"]
                        LCtime.add_curve(y=smooth(mean,window=smoothing_window),std=std,\
                                            label=f"agent = {agent}, α = {alpha}, ε= {epsilon}",annotation=False) # add time curve to plot
