...
# agent_list = ["Dyna_LLM","Q-learning","Dyna_Q"] # hier
agent_list = ["Dyna_LLM"]
...
total_progres_should_be = len(agent_list)*len(env_list)*n_repetitions
...
def experiment():
    ...
    # get the indexes (the x-axis) for all the reward dataframes
    ...
    for agent in agent_list: # select the agent for the experiments
        for env in env_list: # select environment for the experiments
            if agent == "Dyna_LLM":
                run_repetitions(n_timesteps=5,eval_interval=eval_interval,n_repetitions=llm_n_repetitions,\
    ...
    # subprocess.run(["python", "plotresults.py"]) #hier uncomment this
